...
def user_input(user_question):
    try:
        ...
        print(response)
        st.write("Reply: ", response["output_text"])
        return response['output_text'][:50]
    except Exception as e:
        st.error(f"Error with PDF interaction: {e}")
        return None
...
def search_articles(query):
    conn = http.client.HTTPSConnection("google-web-search1.p.rapidapi.com")
    ...
    try:
        conn.request("GET", f"/?query={query_encoded}&limit=20&related_keywords=true", headers=headers)
        res = conn.getresponse()
        data = res.read()
        
        # Debugging - print the response
        print("Web Search API Response:", data.decode("utf-8"))
        
        search_results = json.loads(data.decode("utf-8"))
        return search_results.get("results", [])
    except Exception as e:
        st.error(f"Error fetching Google web search results: {e}")
        return []


def display_articles(query):
    if query:
        st.subheader(f"Related Articles 📝")
        articles = search_articles(query)
...
def display_youtube_videos(query):
    if query:
        st.subheader(f" Related YouTube Videos 🎥")
        videos = get_youtube_videos(query)
...
def main():
    ...
    if user_question:
        query = user_input(user_question.strip())
        if query:  
            display_youtube_videos(query)
            display_articles(user_question)
    with st.sidebar:
        st.title("Menu:")
        pdf_docs = st.file_uploader("Upload your PDF Files and Click on the Submit & Process Button", accept_multiple_files=True)
